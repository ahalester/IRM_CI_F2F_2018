...
public class SimpleSearchByEnergyFlux extends TestBase {

	@Test
	public void Search_FilterByFlux_ReturnsSameNumberOfRowsAsInDB()
			throws Exception {
		// Arrange
		...
		String query = "SELECT count(measurement_id) FROM "
				...
				+ " AND valid = 1 "
				+ "    AND (type_id = 1 )                                                "
				+ "    AND flux     > = " + properties.min(validFlux) + "                                               "
				+ "    AND flux    <= " + properties.max(validFlux) + "                                                "
				+ "    ) q                                                               "
				+ "  ) part                                                              "
	...
	@Test(enabled = false)
	public void Search_FilterByFlux_ReturnsNOResults() {
		// Arrange
		queryFormUtil.insertValue(EnergyComponentIDs.fluxId,
