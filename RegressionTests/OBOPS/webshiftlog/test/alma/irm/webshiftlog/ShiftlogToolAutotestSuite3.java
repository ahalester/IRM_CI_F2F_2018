/*
 * NAME: ShiftlogToolAutotestSuite3.java
 * AUTHOR: Kyoko Nakamura
 * DATE: 2015-09-14
 * 
 * PURPOSE: 
 *  Verification of WebSLT with reports.
 *  
 ...
 * 2018-04-27	knaka	Added WaitTime definition
 * 2018-04-27	knaka	Added a check if there's data in weatherReport()
 * ----------------------------------------------------------------
 */
...
public class ShiftlogToolAutotestSuite3 {
	...
	// 17-04-27 Indefinite is erased (ICT-9798)
	...
	public void projectReportMatching(){
		...
		for(int i = 0; i < pcodeArray.length; i++){
			...
			// 17-03-08: The last loop is not executed for avoiding "9999.4.00001.CSV 9999.4.00001.CSV"
			if(i != pcodeArray.length-1){
	...
	public void weatherReport(){
	    ...
	    for(int i = 0;i < intervalOption.length -1;i++){	
			...
			for(int pos = formatHTMLPos; pos <= formatHTMLPos; pos++){
		    	...
		    	if(pos == formatHTMLPos){
		    		// 18-04-27 Check if there's plot data or not
		    		if(baseProc.checkText(cmt, "No Data")){
		    			log(TUT + "WebSLT shows \"No data\".");
		    		}
		    		else{
	...
	public void executionReport(){
		...
		while(true){
			...
			popProc.waiting(cmt,WaitTime);				// Need to wait at least 0.1 sec for creating of a file
		}
