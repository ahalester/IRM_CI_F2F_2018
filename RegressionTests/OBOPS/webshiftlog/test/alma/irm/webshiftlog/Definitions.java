/*
 * NAME: Definitions.java
 * AUTHOR: Kyoko Nakamura
 * DATE: 2015-09-14
 * 
 * PURPOSE: 
 *  Definitions of the common for all Suites.
 *  
 * PRE- AND POST-CONDITIONS:
 *  
 * MAINTENANCE:
 * - The name of variables must be capital letters.
 * - The ZK locators must begin with "Z_"
 * - "USER DEFINED PARAMETERS" must change according to the running environment.
 * - The methods of logging and assertion are defined here, because all Suites
 *  use them. 
 *   
 * ----------------------------------------------------------------
 * AMENDEMENTS
 ...
 * 2017-07-13	knaka	Added Z_SORTCARET which appears after sorting
 * 2018-04-27	knaka	Added wait time amounts
 * 2018-05-21	knaka	Added Typenum to get position and text name
 * ----------------------------------------------------------------
 */
...
//	static final String FORMAT_SEARCH = "yyyy/MM/dd hh:mm:ss a";// Format of Interval Begin/End on Search popup
	...
	static final String CHECK_EXECINF_PANE = OBSCYCLEMENU;		// To confirm Exec.Inf. pane
	static final String CHECK_ENGINEER_PANE = "Antenna control";// To confirm Engineer pane
	...
	static final String	Z_SORTCARET = "z-icon-caret-up";		// Caret appeared after sorting, 17-07-13 created
	...
	static final String[] CLMS = new String[]{					// All columns on Entries
		"Type"      ,"Timestamp" ,"Location","ArrayName","Project code", // Added ArrayName at 17-03-03
		"SchedBlock","ExecBlock" ,"Status"  ,"QA0"      ,"Subject"     ,
		"Author"    ,"Array Activity"  ,"Attachment","Calibration flag"}; // Added Activity at 17-03-03, then modified at 18-03-08
	...
	static final int CLM_ANAME = CLM_LOC + 1;					// Column: ArrayName added at 17-02-16
	static final int CLM_PCODE = CLM_ANAME + 1;					// Column: Project code
	...
	static final int CLM_ACTIVITY= CLM_AUTHOR + 1;				// Column: Activity added at 17-03-03
	static final int CLM_ATTACH = CLM_ACTIVITY + 1;				// Column: attachment
	...
	static enum Typenum{										// Types enum at 28-05-21
		ANTEN(0,"ANTEN"), AOG(1,"AOG"), ARRAY(2,"ARRAY"), 
		CMDLN(3,"CMDLN"), DOWN(4,"DOWN"), ENGIN(5,"ENGIN"),
		ENTRY(6,"EMTRY"), MMEX(7,"MMEX"), SBEX(8,"SBEX"),
		SHIFT(9,"SHIFT"), WEATH(10,"WEATH");
		private final int id;
		private final String str;
		private Typenum(final int id, final String str){ this.id = id; this.str = str; }
		public int getId(){ return this.id; }					// Get number (= Type's position)
		public String getStr() { return this.str; }				// Get Type name
	}
	...
	// Wait time
	static final int ONESEC = 1000;								// Wait for 1000msec
	static final int HALFSEC = 500;								// Wait for 500msec
	static final int FEWSEC = 15;								// Wait for a few milliseconds
	...
	//---  from HERE ---
	//[URLs]
	static final String BASEURL = "http://irm-docker.mtk.nao.ac.jp:10080/webshiftlog/"; 	// Identical to the one in BaseWindowProcedure
	static final String STATUSURL = BASEURL + "status.zul";
	static final String ENFORCED_LOGOUT_URL = "https://irm-docker.mtk.nao.ac.jp:10443/cas/logout";
	//[Accounts]
	static final String USERNAME = "knaka";
	static final String PASSWORD = "Ky0k0";
	static final String USERNAME_WO_AUTH = "almaproc";
	static final String PASSWORD_WO_AUTH = "Almapr0c";
	...
	static final String ACT_BEGIN = "2016-04-15 00:00:00";		// Begin in Interval "Other" for Activity, 18-03-08
	static final String ACT_END = "2016-04-20 23:59:00";		// End in Interval "Other" for Activity, 18-03-08
	static final String INDEF_BEGIN = "2010-01-01T00:00:00";	// Begin with "Indefinite", added at 17-03-03
