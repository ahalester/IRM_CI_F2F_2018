/*
 * NAME: PopupProcedure.java
 * AUTHOR: Kyoko Nakamura
 * DATE: 2015-09-14
 * 
 * PURPOSE: 
 *  A set of common methods relate to the popup windows. 
 *  
 ...
 * 2018-04-27	knaka	Changed arguments of sleep() to defined variables
 * ----------------------------------------------------------------
 */
...
class PopupProcedure {
	...
	void closePopup(CommonTools cmt,String cnameP, String cnameC,int n,String text){
	    ...
	    try{
	    	...
	    	cmt.waitForTextIsUnloaded(text,8);
		}catch(Exception e){
			saveLog(e);
		}
    	...
    	if(n == 3){		// Check only at clicking [Search], not [Cancel]
    		try{
	    		cmt.waitForTextIsUnloaded(loadMsg,8);
	...
	void clickItemAndWait(CommonTools cmt,String cnameP, String cnameC, int nP, int nC, int flag){
		...
		try{
			if(flag == 1)	// wait until the check mark is appeared
				...
				log(myInfo + "...Just sleep for " + FEWSEC + "/1000.s...");
				cmt.sleep(FEWSEC);
			}	
		}catch(Exception e){
			saveLog(e);
	...
	void waitAppearance(CommonTools cmt,String text){
		...
		try{
			cmt.waitForTextIsLoaded(text,8);	
		}catch(Exception e){
			saveLog(e);
		}
	}
