...
/*--------------------------------------------------------------------------------
 *  TestSuite 1: Verify WebSLT in general
 *--------------------------------------------------------------------------------*/  
...
public class ShiftlogToolAutotestSuite1 {
	...
	private final String casTitle = "CAS – Central Authentication Service";	// CAS title
	...
	private final String almaportalTitle = "ALMA Science Portal";		// ALMA portal title
	...
	@Ignore			// (2015-04-30) refresh() has not been implemented yet
	@Test
	@Order(order=8)
	...
	public void loginWithAuth(){
	    ...
	    doAssertThat(driver.getTitle(),is(casTitle));
	...
	public void statusPage(){
	    ...
	    doAssertThat(baseProc.checkText(cmt,"User Information"),is(equalTo(true)));
	...
	public void almaPage(){
	    ...
	    baseProc.login(cmt,WEBTITLE,USERNAME,PASSWORD);
		baseProc.goToAlmaPage(cmt,Z_TOOLBAR_BTN,BASE_ALMAPORTAL,almaportalTitle);
	    ...
	    doAssertThat(driver.getTitle(),is(almaportalTitle));
	...
	/*
	 * TODO(03-25) How should it be confirmed the refresh is done? Thinking...
	 */
	public void refresh(){
		log(TU);
		log("## " + new Throwable().getStackTrace()[0].getMethodName() + " ##");
	...
	public void goAndBackPage(){
		...
		doAssertThat(popProc.stateInSelection(cmt,Z_TABLECHILD,Z_TYPECHKBOX,GENERAL,TYPES_SHIFT),is(equalTo(true)));
	...
	public void sortColumns(){
		...
		int asc = 0;			// Use in ascending order
		int des = 1;			// Use in descending order
    	...
    	if(baseProc.isNoEntry(cmt,Z_ENTRY_INFO,NO_ENTRY)) log("Abort the test due to no entries");
    		...
    		for(int i = 0; i < CLMS.length - 2; i++){
    			...
    			for(int sort = asc; sort <= des; sort++){
    				baseProc.sortList(cmt,Z_PANELCHILD,Z_LISTHEAD,i);
	...
	public void linkToAqua(){
		...
		String aquaTitle = "ALMA QUality Assurance Tool (AQUA)";
    	...
    	if(baseProc.isNoEntry(cmt,Z_ENTRY_INFO,NO_ENTRY)) log("Abort the test due to no entries");
    		...
    		if(row == rowMax) log("Cannot get EB uid. Test is aborted!");
    			...
    			doAssertThat(driver.getTitle(),is(aquaTitle));
