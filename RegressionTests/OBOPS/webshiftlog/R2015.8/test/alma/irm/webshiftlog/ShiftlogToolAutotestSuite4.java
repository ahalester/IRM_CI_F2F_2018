...
/*--------------------------------------------------------------------------------
 *					TestSuite 3: Verify WebSLT with entry reports
 *--------------------------------------------------------------------------------*/
...
public class ShiftlogToolAutotestSuite4 {
	...
	public void sortsOfReport(){
	    ...
	    // Procedure: Click a type one by one and check the creatable reports
	    for(int typeId = 0; typeId < TYPE_NAME.length; typeId++){
			...
			for(n = 0; n < labelTopPopup.length-1; n++){	// last one is "menuitem" and not open any popup
				item = popProc.getText(cmt,zClassMenuP,zClassMenu,topPopup,n,REPPOPUP);
				doAssertThat(item,is(labelTopPopup[n]));	
				judge = baseProc.isMenuSelectable(cmt,zClassMenuP,zClassMenu,topPopup,n,"class");
				doAssertThat(judge,is(equalTo(true)));
			}
			item = popProc.getText(cmt,zClassMenuP,zClassMenuItem,topPopup,0,REPPOPUP); // "menuitem" located at 4th
			doAssertThat(item,is(labelTopPopup[n]));	
			judge = baseProc.isMenuSelectable(cmt,zClassMenuP,zClassMenuItem,topPopup,0,"class");
			if(typeId == TYPES_SHIFT) doAssertThat(judge,is(equalTo(true)));	// Only SHIFT is available
			...
			for(int i = 0; i < labelTopPopup.length-1; i++){	// Looping except the last one
				...
				for(String expected: labelChildPopup[i]){
					item= popProc.getText(cmt,zClassMenuP,zClassMenuItem,childId,j,REPPOPUP);
					doAssertThat(item,is(expected));
			...
			// Procedure: Refresh and open search popup and clear the previous check mark
	    ...
	    for(int typeId = 0; typeId < TYPE_NAME.length; typeId++){
			...
			for(int fmt = 0; fmt < 2; fmt++){ // On the top menu, "0" is PDF format and "1" is HTML
				...
				for(int id = 0; id < 2; id ++){	// On a child menu, "0" is selected entry and "1" is all
			    	...
			    	if(fmt == 1 && typeId != TYPES_WEATH){	// WEATH does not show any entries, so ignore it
