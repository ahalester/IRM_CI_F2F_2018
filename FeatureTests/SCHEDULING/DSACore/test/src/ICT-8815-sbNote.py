...
class ICT8815sbNote(DsaWebServiceTestCase.DsaWebServiceTestCase):
    
    def test_checkColumnPresent(self):
        '''
        ICT-8815: Check is a response from the server for a run() includes a column named sbNote
        ...
        if response == None:
            ...
            columnFound=False
            for sb in j.items():
                if 'sbNote' in sb[1].keys():
                    columnFound = True
            if not columnFound:
                self.fail("Response from DSACore service does not include a sbNote column. Please check that the correct version is deployed (APR2017+)")
