...
import cl.apa.web.utils.CaptureScreenshotUtil;
import cl.apa.web.utils.EncryptUtil;
import cl.apa.web.utils.PropertiesFileUtil;
import cl.apa.web.utils.WaitUtil;
...
//                }
            ...
            baseUrl = PropertiesFileUtil.getNavigationURL("url");
            driver = new WebDriverExtended(baseWebDriver, baseUrl);
            driver.manage().timeouts().implicitlyWait(WaitUtil.DEFAULT_WAIT_FOR_ELEMENT, SECONDS);
            driver.manage().timeouts().pageLoadTimeout(WaitUtil.DEFAULT_WAIT_FOR_PAGE_LOAD, SECONDS);
    ...
    public void tearDown(Scenario scenario) {
        ...
        if (scenario.isFailed()) {
            ...
            if ("yes".equalsIgnoreCase(grid)) {
                ...
                CaptureScreenshotUtil.captureScreenshot(driver, scenarioName, scenario);
            }
    ...
    private WebDriver firefoxSetup(String grid, String docker, String selenium) throws MalformedURLException {
        ...
        if ("3".equals(selenium)) {
            setWebDriverBinaryPath("webdriver.gecko.driver",
                    PropertiesFileUtil.getProperty("webdriver.firefox.windows"),
                    PropertiesFileUtil.getProperty("webdriver.firefox.mac"),
                    PropertiesFileUtil.getProperty("webdriver.firefox.linux"));
        ...
        if ("yes".equalsIgnoreCase(grid)) {
            ...
            baseWebDriver = new FirefoxDriver(options);
            //baseWebDriver = new FirefoxDriver(fdc);
            baseWebDriver.manage().window().maximize();
    ...
    private WebDriver chromeSetup(String grid, String docker, String selenium) throws MalformedURLException {
        ...
        if ("3".equals(selenium)) {
            setWebDriverBinaryPath("webdriver.chrome.driver",
                    PropertiesFileUtil.getProperty("webdriver.chrome.windows"),
                    PropertiesFileUtil.getProperty("webdriver.chrome.mac"),
                    PropertiesFileUtil.getProperty("webdriver.chrome.linux"));
        } else {
            //                    TODO - add the webdriver v2.0 path
        }
        ...
        if ("yes".equalsIgnoreCase(grid)) {
            ...
            //baseWebDriver = new ChromeDriver(cdc);
            LOG.info("****************** Running tests on the localhost under "
