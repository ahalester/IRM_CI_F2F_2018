...
public class StringUtil {
    ...
    public static String absoluteSshCmd(String sshCmd, String relativeCmd) {
        String absoluteCmd = "";
        try {
            if (ReadWriteTextToFileUtil.readFile(relativeCmd).contains("http")) {
                absoluteCmd = sshCmd + removeHttpAndPort(ReadWriteTextToFileUtil.readFile(relativeCmd));
            } else {
                absoluteCmd = sshCmd + ReadWriteTextToFileUtil.readFile(relativeCmd);
            }
    ...
    public static String removeGenericsFromSshCmd(String sshCmd, String gatewayMachine,
                                                  String innerMachine, String innerCmd) {
        try {
            innerMachine = removeHttpAndPort(ReadWriteTextToFileUtil.readFile(innerMachine));
            sshCmd = sshCmd.replace("<gateway_machine>", ReadWriteTextToFileUtil.readFile(gatewayMachine));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static String fillSystemRegisterCmdWithParams(String sshCmd, String endpointIp,
                                                         String endpointPass, String serverIp,
                                                         String apiKey) {
        sshCmd = sshCmd.replace("<provide_endpoint_ip>", endpointIp);
        sshCmd = sshCmd.replace("<provide_password>", endpointPass);
        try {
            sshCmd = sshCmd.replace("<provide_SC_server_ip>", ReadWriteTextToFileUtil.readFile(serverIp));
        } catch (IOException e) {
            e.printStackTrace();
        }
        try {
            sshCmd = sshCmd.replace("<provide_SC_api_key>", ReadWriteTextToFileUtil.readFile(apiKey));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return sshCmd;
    ...
    public static String fillFileDownloadCmdWithParams(String sshCmd, String fileName,
                                                       String serverIp, String systemId,
                                                       String apiKey) {
        ...
        try {
            sshCmd = sshCmd.replace("<generated_system_id>", ReadWriteTextToFileUtil.readFile(systemId));
        } catch (IOException e) {
            e.printStackTrace();
        }
        try {
     ...
     *
     * @param list  the list of String elements (List<String>) to be joined in a string
    ...
    public static String currentDate(long timeInMillis) {
        return new SimpleDateFormat("ddMMyyyy").format(new Date(timeInMillis));
    }

    public static String getLastTestResult(String fileName) {
        String fileContent = null;
        try {
            fileContent = ReadWriteTextToFileUtil.readFile(fileName);
        } catch (IOException e) {
    ...
    public static int lastIndexOfSubstring(String stringBuilder) {
        ...
        while (index != -1) {
            ...
            index = sb.indexOf(findStr, index);
            if (index != -1) {
                lastIndex = index;
                LOG.info("Last index of the current timezone within the string builder is '%d' ",
                        lastIndex);
            }
            ...
            if (index != -1) {
                ...
                if (index != -1) {
                    // lastIndex = index;
                    LOG.info(String.format("Last index of the substring is '%d'", lastIndex));
                }
            }
        }
        return lastIndex;
    ...
    public static String rgbToHex(WebElement we, String rgbValues) {
        String[] numbers = rgbValues.replace("rgba(", "").replace(")",
                "").split(",");
        int r = Integer.parseInt(numbers[0].trim());
        int g = Integer.parseInt(numbers[1].trim());
        int b = Integer.parseInt(numbers[2].trim());
        LOG.info("The RGB color code for the web element " + we + " is r: " + r + "g: " + g
                + "b: " + b);
    ...
    public static String rgbToHex(String we, String rgbValues) {
        String[] numbers = rgbValues.replace("rgba(", "").replace(")", "")
                .split(",");
        int r = Integer.parseInt(numbers[0].trim());
        int g = Integer.parseInt(numbers[1].trim());
    ...
    public static String keyValidator() {
        Timestamp timestamp = new Timestamp(System.currentTimeMillis());
        Date date = new Date(timestamp.getTime());
    ...
    public static Date dateConverter(String dateFormat) {
    ...
    public static String extractMonthOrYearFromCurrentDate(String monthOrYear) {
        ...
        if (monthOrYear.equalsIgnoreCase("YYYY")
                || monthOrYear.equalsIgnoreCase("YYY")
    ...
    public static String extractLanguageFromUrl(String url) {
        return url.substring(url.indexOf("language="), url.indexOf("language=")
                + "language=".length() + 2).replace("language=", "");
    }

    public static String convertDateFormat(String dateToConvert) {
        return dateToConvert.replaceAll("(\\d+)/(\\d+)/(\\d+)",
                "$3/$2/$1").replace("/", "-");
    }

    public static String updateDatesInUrl(String url) {
        if (url.contains("arrival_date") && url.contains("departure_date")) {
            String arrivalDate = url.substring(url.indexOf("arrival_date="),
                    url.indexOf("arrival_date=") + "arrival_date=".length() + 10);
            url = url.replace(arrivalDate, updateDate(arrivalDate));
            String departureDate = url.substring(url.indexOf("departure_date="),
                    url.indexOf("departure_date=")
    ...
    public static void checkIfStringMatchesPattern(String input, String pattern) {
        Pattern ptrn = Pattern.compile(pattern);
        Assert.assertTrue(String.format("The displayed string '%s' doesn't match the pattern '%s'!",
                input, pattern), ptrn.matcher(input).matches());
    }
