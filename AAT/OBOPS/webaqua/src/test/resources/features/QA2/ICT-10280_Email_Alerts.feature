...
# https://ictjira.alma.cl/browse/ICT-10787
  ...
  notification will be sent
    ...
    And the user navigates to a specific URL
      | url |
#    Then QA2 test environment is available
    Then the test environment is available
      | QA2 |
    And AQUA login page is displayed
    When the user fills the credentials
      | username_obops |
...
# https://ictjira.alma.cl/browse/ICT-12235
  @all @email @2018apr
  Scenario: Email notification subject contains the state name when the state changes
    When the user selects a specific option
      | ous_state | reviewing |
    And the user clicks on a specific field
      | processing_flags |
...
#    TODO - ugly workaround | to be removed
    ...
    When the user navigates to a specific URL
      | mail_hog_url |
    And the email notification subject contains specific data
      | [AQUA]                                  |
      | Project code                            |
      | SB name(s)                              |
      | Verified                                |
      | failed                                  |
      | Ingestion of PIPELINE products for MOUS |
    When the user opens a specific email message
      | Project code |
      | SB name(s)   |
    Then the email body contains the specific dynamic text
