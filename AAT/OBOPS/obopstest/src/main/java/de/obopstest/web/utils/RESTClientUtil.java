...
public class RESTClientUtil {
        ...
        HostnameVerifier hv = new HostnameVerifier() {
            public boolean verify(String urlHostName, SSLSession session) {
                if (!urlHostName.equalsIgnoreCase(session.getPeerHost())) {
                    LOG.info("Warning: URL host '" + urlHostName + "' is different to SSLSession "
    ...
    public static void apiCall(String serverRootUri, List<String> proxy, String callType,
                               String jsonBody, int responseCode) {
        apiCall(serverRootUri, callType, jsonBody, null);
    }

    public static URLConnection setUsernamePassword(URL url, String username, String pasword)
            throws IOException {
