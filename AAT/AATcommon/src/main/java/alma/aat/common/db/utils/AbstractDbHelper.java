...
//        init();
    ...
    public AbstractDbHelper(String url, String user, String password) {
        init(url, user, user);
    }
    ...
    public List<Map<String, String>> getRecordList(String query, String... params) {
        try {
            List<Map<String, String>> result = getJdbcTemplate().queryForList(query,params);
            LOG.info(LOG_MESSAGE + query);
            return result;
        } catch (SQLException e) {
            LOG.error(ERROR_MESSAGE + query);
    ...
    public Map<String, String> getSingleRecord(String query, String... params) {
        try {
            Map<String, String> result = getJdbcTemplate().queryForList(query, params).get(0);
            LOG.info(LOG_MESSAGE + query);
            return result;
        } catch (SQLException e) {
            LOG.error(ERROR_MESSAGE + query);
    ...
    public String getSingleValue(String query, String... params) {
        try {
            Map<String, String> resultMap = getJdbcTemplate().queryForList(query,params).get(0);
            String columnName = query.trim().split(" ")[1];
            if (columnName.contains("(")) {
                //get column name if there is any sorting
                columnName = columnName.split("\\(")[0];
...
//    }
    ...
    public void init(String url, String user, String password) {
        this.template = new JdbcTemplate(getConnection(url, user, password), 0);
    }

    //    protected abstract Connection getConnection();
    protected abstract Connection getConnection(String url, String user, String password);
}
